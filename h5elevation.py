...
workers = 1 # Changed to 1 to simplify result aggregation for multiple masks

# Define usecols globally and consistently
...
def prepare_feather_compatible_df(df):
    ...
    # Convert categorical columns to string for Feather compatibility
    for col in df_feather.columns:
        if pd.api.types.is_categorical_dtype(df_feather[col]):
            df_feather[col] = df_feather[col].astype(str)
    
    # Ensure all integer columns use supported dtypes
    # Feather supports int8, int16, int32, int64, uint8, uint16, uint32, uint64
    for col in df_feather.columns:
        if pd.api.types.is_integer_dtype(df_feather[col]):
            # Keep the optimized integer types as they are Feather-compatible
            pass
        elif pd.api.types.is_float_dtype(df_feather[col]):
            # Feather supports float32 and float64
            pass
...
def save_to_formats(df, base_path, basename, file_version, elmask_val):
    ...
    for fmt, info in formats.items():
        try:
            ...
            # Ensure an entry exists, even if failed, for consistent aggregation
            results[fmt] = None 
            continue
    ...
    try:
        ...
        if v == '326':
            ...
            if os.path.exists(outfile):
                os.remove(outfile)
            return None # Return None if version is unknown

        if df_raw is None: # If df_raw couldn't be loaded by any reader
            print(f"  Could not load data from {os.path.basename(file)}. Skipping.")
            if os.path.exists(outfile):
                os.remove(outfile)
            return None

        # Apply usecols and optimize dtypes for standard formats
        if is_std_format:
            df_raw = df_raw[usecols] # This is where usecols is applied
            # Add filename as categorical only for standard formats for consistency
            df_raw['fname'] = pd.Categorical([os.path.basename(outfile)] * len(df_raw))
            df_raw = optimize_dtypes(df_raw)
        else: # For vscintpi1, add fname but don't apply usecols or optimize dtypes based on standard columns
            df_raw['fname'] = pd.Categorical([os.path.basename(outfile)] * len(df_raw))
            # You might still want to optimize dtypes based on its own columns if known
            df_raw = optimize_dtypes(df_raw) # Call optimize_dtypes, it will handle columns present
        ...
        for elmask_val in elevation_masks:
            ...
            df_to_save = df_raw.copy() # Start with a copy of the raw/processed df
            
            if is_std_format:
                # Apply elevation mask only if it's a standard format and 'elev' column exists
            ...
            del df_to_save # Clean up memory
        ...
        if outfile and os.path.exists(outfile): # Clean up unzipped file even on error
            os.remove(outfile)
        return None
    ...
    for file_results_by_mask in all_processed_results:
        if file_results_by_mask:
            for elmask_val, format_results in file_results_by_mask.items():
                if format_results: # Ensure format_results is not None (e.g., if save failed)
                    if elmask_val not in aggregated:
                        aggregated[elmask_val] = {}
                    for fmt, data in format_results.items():
                        if data: # Ensure data for this format is not None
...
# MODIFIED create_single_comparison_plot function
def create_single_comparison_plot(aggregated_data):
    ...
    # Only one subplot now, make it wider
    fig, ax = plt.subplots(1, 1, figsize=(14, 7), constrained_layout=True)
    
    # More distinct colors for more masks - EXTENDED
    mask_colors = {
    ...
    # Get average size for the 0-degree mask to use as baseline
    avg_size_el0 = 1.0 # Initialize to avoid division by zero
    ...
    # Data for plotting (all specified masks) - EXTENDED
    ...
    for mask in comparison_masks:
        if mask in aggregated_data and selected_format in aggregated_data[mask] and aggregated_data[mask][selected_format]['sizes']:
            ...
            percentage = (avg_size / avg_size_el0) * 100 if avg_size_el0 > 0 else 0 # Handle division by zero
    ...
    # --- Plot: Compressed Size % Relative to 0-degree Mask ---
    ...
    ax.set_xlabel('Elevation Mask Angle') # Added x-axis label for clarity
    ax.set_xticks(x)
    ax.set_xticklabels(mask_labels)
    # Ensure ylim always has a range
    y_max = max(size_percentages) * 1.15 if size_percentages and max(size_percentages) > 0 else 100
    ax.set_ylim(0, y_max)
    
    for i, bar in enumerate(bars):
        if size_percentages[i] > 0:
            ax.text(bar.get_x() + bar.get_width()/2., bar.get_height() + (y_max * 0.01), # Dynamic text position
...
if __name__ == '__main__':
    ...
    # MODIFICATION HERE: Added 30, 35, 40 to the list of elevation masks
