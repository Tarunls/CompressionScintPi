...
def save_to_formats(df, base_path, basename, file_version):
    ...
    formats = {
        'parquet_zstd':   {'ext': '.parquet', 'folder': 'pq_zstd',   'compression': 'zstd', 'compression_level': 9}, 
        ...
        'duckdb_zstd':    {'ext': '.duckdb',   'folder': 'duckdb_zstd', 'compression': 'zstd', 'compression_level': 9}, 
        'duckdb_lz4':     {'ext': '.duckdb',   'folder': 'duckdb_lz4',  'compression': 'lz4'},
        'feather_zstd':   {'ext': '.feather', 'folder': 'feather_zstd', 'compression': 'zstd', 'compression_level': 9}, 
        'feather_lz4':    {'ext': '.feather', 'folder': 'feather_lz4',  'compression': 'lz4'},
        'orc_zstd':       {'ext': '.orc',     'folder': 'orc_zstd',    'compression': 'zstd', 'compression_level': 9}, 
        'orc_snappy':     {'ext': '.orc',     'folder': 'orc_snappy',  'compression': 'snappy'}, 
        'hdf5_blosc':     {'ext': '.h5', 'folder': 'hdf5_blosc', 'compression': 'blosc', 'compression_level': 9}, 
            ...
            if fmt.startswith('parquet'):
                ...
                for col in df_hdf5.columns:
                    if pd.api.types.is_categorical_dtype(df_hdf5[col]):
                        df_hdf5[col] = df_hdf5[col].astype(str) 
...
def read_from_formats(saved_file_paths):
    ...
    for fmt, filepath in saved_file_paths.items():
        ...
        try:
            ...
            read_speed_mbps = file_size_mb / read_time if read_time > 0 else np.inf 
...
def create_conversion_plots(organized_data, formats, file_types, zipped_original_sizes):
    ...
    selected_formats_for_type_comparison = ['parquet_zstd', 'duckdb_zstd', 'feather_lz4', 'orc_zstd'] 
    group_width = 0.8
    ...
    ax9.legend(loc='upper left', fontsize=8, ncol=len(selected_formats_for_type_comparison)) 
